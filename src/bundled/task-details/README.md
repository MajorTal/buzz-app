...
Task references and Projects entries open the task workspace, with a persistent header and the same conversation underneath. **Edit task** opens the existing editor; **View in channel** returns to the original channel and thread. The workspace does not create another conversation or change how replies are signed and delivered. Its header shows the recorded assignee and branches; PR discovery and task-status fields are not implemented.
